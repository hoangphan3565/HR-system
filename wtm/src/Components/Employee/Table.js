...
const Table = (props) => {
  ...
  const optionsDept = deptOptions.map((e) => {
    return <Option value={e}>Department 0{e}</Option>;
  });
  ...
  const employee = currentEmployees.map((e, index) => {
    return <Item e={e} test={transmiss} />;
  });
  const layout = {
    labelCol: { span: 6 },
    wrapperCol: { span: 18 },
  ...
  const onFinish = () => {
    ...
    EmployeeServices.add(
      department.current.props.value,
      position.current.props.value,
      employee
    ).then((res) => {
      setLink("1");
      setLink("");
                ...
                >
                  <Form {...layout} onFinish={onFinish}>
                    ...
                    <Form.Item label="Department" name="department">
                      <Select ref={department}>{optionsDept}</Select>
                    </Form.Item>
                    <Form.Item label="Position" name="position">
                      <Select ref={position}>{optionsPos}</Select>
                    </Form.Item>
          ...
          <div className="card-body">
            <div className="table-responsive-md">
              <table className="table table-striped table-hover">
                ...
                <tbody>{employee.length !== 0 ? employee : <Empty />}</tbody>
              </table>
              <Pagination
                showSizeChanger
                current={currentPage}
